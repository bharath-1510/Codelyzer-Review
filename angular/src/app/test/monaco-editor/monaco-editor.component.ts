...
export class MonacoEditorComponent implements OnInit, OnDestroy {
    ...
    if (this.editor) {
      this.editor.getModel()?.onDidChangeContent(() => {
        if (this.editor) {
          ...
          const a:string="-- Default Code"
          if(currentCode!=a){

          // Check if the current code contains the boilerplate code
          if (currentCode.length < boilerplateCode.length) {
            // Restore the boilerplate code
            this.editor.setValue(boilerplateCode);
          }
        }
      }
      });
  ...
  onLanguageChange() {
    ...
    console.log('in change the selected language is',this.selectedLanguage);
  ...
  change(question: Question) {
    ...
    // if(this.selectedQuestion.templates==undefined){this.selectedLanguage='mysql'}
    ...
    if (code.language.trim().toLowerCase()  === 'mysql') {
      ...
      this.submitService.submitCode(code).subscribe(
        ...
        (error) => {
          ...
          alert('Error in submitting , submit again');
        }
      );
    }
  }
